...
public class PremiumLogic implements Logic {
    ...
    public QueuePlayer priorityLogic(QueueServer server, AdaptedPlayer player) {
        ...
        if(
                player.hasPermission("simplequeue.bypass") ||
                player.hasPermission("ajqueue.bypass") ||
                player.hasPermission("simplequeue.serverbypass."+server.getName()) ||
                player.hasPermission("ajqueue.serverbypass."+server.getName()) ||
                player.hasPermission("simplequeue.joinfullandbypassserver."+server.getName()) ||
                ...
                permissionGetter.hasUniqueFullBypass(player, server.getName())
        ) {
            if(debug) {
